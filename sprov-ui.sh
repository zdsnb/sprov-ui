...
#   Github - sprov-ui: https://github.com/zdsnb/sprov-ui
#======================================================
...
install() {
    install_base
    bash <(curl -L -s https://github.com/zdsnb/sprov-ui/raw/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
uninstall() {
    ...
    echo -e "Github issues: ${green}https://github.com/zdsnb/sprov-ui/issues${plain}"
...
install_bbr() {
    bash <(curl -L -s https://github.com/zdsnb/sprov-ui/raw/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
